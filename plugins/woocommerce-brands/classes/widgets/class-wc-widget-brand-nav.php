			...
			if ( $display_type == 'dropdown' ) {
				...
				if ( $current_tax && $taxonomy == $current_tax ) {
					...
					foreach ( $terms as $term ) {
						...
						echo '<option value="' . $term->term_id . '" '.selected( isset( $_GET[ 'filter_brand' ] ) ? $_GET[ 'filter_brand' ] : '' , $term->term_id, false ) . '>' . $term->name . '</option>';
					}
					...
					$woocommerce->add_inline_js("
						...
						jQuery('#dropdown_layered_nav_$taxonomy_filter').change(function(){
							...
							location.href = '" . add_query_arg('filtering', '1', remove_query_arg( 'filter_brand' ) ) . "&filter_brand=' + jQuery('#dropdown_layered_nav_$taxonomy_filter').val();
				...
				foreach ( $terms as $term ) {
					...
					$current_filter = ( isset( $_GET[ 'filter_brand' ] ) ) ? explode( ',', $_GET[ 'filter_brand' ] ) : array();
					...
					if ( isset( $_chosen_attributes['filter_brand'] ) && is_array( $_chosen_attributes['filter_brand']['terms'] ) && in_array( $term->term_id, $_chosen_attributes['filter_brand']['terms'] ) ) {
						...
						if ( sizeof( $current_filter ) > 1 ) {
							...
							$link = add_query_arg( 'filter_brand', implode( ',', $current_filter_without_this ), $link );
						}
...
function woocommerce_brands_layered_nav_init( $filtered_posts ) {
	...
	if ( is_active_widget( false, false, 'woocommerce_brand_nav', true ) && ! is_admin() ) {
		...
		if ( ! empty( $_GET[ 'filter_brand' ] ) ) {
			...
			$terms 	= array_map( 'intval', explode( ',', $_GET[ 'filter_brand' ] ) );

			if ( sizeof( $terms ) > 0 ) {

				$_chosen_attributes['filter_brand']['terms'] = $terms;
				$_chosen_attributes['filter_brand']['query_type'] = 'and';
